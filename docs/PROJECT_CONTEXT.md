# 项目上下文（持续维护）

## 产品边界

本工具只管理数据集及其关系，不执行图片格式转换、数据增强或模型推理。实际转换由其他工具完成，本工具登记转换结果，并把派生数据集关联回原始数据集。

## 当前需求共识

- 原始数据集：只存图片。
- 全部、原始、已处理和创建数据集列表先按 `_YYYY_MMDD_` 前的完整项目名称排列，使相同项目连续显示；同一项目内再按解析出的日期倒序排列。例如 `Corona_2026_0516_aaa` 的项目为 `Corona`、日期为 2026-05-16；不符合规则的记录按更新时间排在有效命名记录之后。
- 外部工具处理后会产生新数据集；处理后的数据集通常含图片和 LabelMe JSON 标签。
- 当前模块：原始数据集、已处理数据集，以及训练集、测试集、验证集三个数据集划分大模块。
- 训练/测试/验证模块下的首个小模块叫“创建数据集”；以后继续增加标签转换、重复检查等小模块。
- 创建划分数据集时可组合多个“已处理数据集 + 标注批次”，每个来源选择固定对数或全部匹配对。
- 初版按路径稳定排序后取前 N 对，不随机、不复制文件；输出 JSON 清单记录连续序号、图片绝对路径、标签绝对路径和来源 ID。
- 划分数据集备注自动列出全部组成来源及实际选取数量，并追加用户补充备注。
- 左侧“创建数据集”模块将划分清单中的文件复制到新的工具托管根目录；默认子目录为 `images` 和 `jsons`，允许改名。
- “创建数据集”的来源清单通过下拉框选择已登记的训练集、测试集或验证集，不再手工选择任意 JSON 文件。
- 复制创建时不覆盖已有根目录；同名样本增加数字后缀，并保持图片与 JSON 主名一致。
- 已创建数据集记录保存来源清单、子目录名、对数和根目录所有权；删除时仅在所有权标记有效且目标不是磁盘根目录时，永久删除实际根目录和记录。
- 左侧“YOLO处理”大模块包含“格式转换”和“重复数据集检查”两个小模块，操作记录单独持久化到 `yolo-operations.json`。
- 左侧“权重记录”模块独立登记 PT、ONNX 等权重文件路径、自动识别格式、备注和修改历史；只管理记录，不复制或删除实际权重文件。
- 格式转换只处理用户输入的类别，类别输入顺序对应 YOLO ID；用户可选择将 LabelMe shape 转成归一化目标检测外接框或语义分割多边形（rectangle 转四角点），保留相对子目录并生成 `classes.txt`。转换后可检查本次生成的空标签，并选择保留或删除。
- 重复检查递归比较两个目录的实际标签 TXT 文件名、忽略大小写且排除 `classes.txt`；删除必须明确选择 A 或 B，只删除命中的 TXT 文件并记录备注。
- 每个数据集必须支持自由备注。
- 原始数据集和图片数据集扫描只处理图片，不自动查找或读取标签。
- 一个已处理数据集可以关联多组标注批次；每组独立保存标签路径、名称、规则备注和统计结果。
- 标注批次仅在新增或用户要求重新统计时读取 LabelMe JSON，并按 `shapes[].label` 统计每类标注实例数量。
- 每个标注批次按不含扩展名的文件名、忽略大小写，将图片与 JSON 一对一匹配，并统计匹配、无图片标签和无标签图片数量。
- 匹配递归枚举图片和标签目录中的实际文件，不把目录名当成图片或标签；旧统计在用户主动统计前显示“尚未统计”。
- 每次修改可填写“本次修改说明”，形成按时间排列的变更记录。
- 项目使用 C#、WPF、C++ 和 OpenCV。
- C++ 使用 CMake 构建 DLL，通过稳定的 C ABI 给 C# P/Invoke 调用。
- 仓库不维护 `.sln`；根目录 `CMakeLists.txt` 是统一构建入口，并通过 .NET SDK 编译 WPF/C# 项目。
- 根 CMake 将 EXE、托管依赖、原生 DLL 和 OpenCV DLL 统一输出到 `bin/<配置>`。
- OpenCV Release 路径：`D:\opencv-4.5.3\opencv-4.5.3\build\install`。
- OpenCV 运行时 DLL 路径：`D:\opencv-4.5.3\opencv-4.5.3\build\install\bin`。
- 当前 OpenCV 安装经过裁剪，模块清单引用了不存在的 dnn 等库；CMake 只显式链接现有的 core、imgcodecs、imgproc Release 库。
- 约定在仓库根目录使用 `cmake -S . -B ./build -G "Visual Studio 17 2022" -A x64` 配置整个工程；`native` 目录仍可独立配置。
- 根目录 `build_release.bat` 是一键 Release 编译入口；正式输出在 `bin/Release`，`build` 仅保存 CMake/Visual Studio 中间文件。
- 根目录 `publish_standalone.bat` 生成 `bin/Standalone` Windows x64 自包含目录，随包携带 .NET 8、C++、OpenCV 和 Visual C++ x64 运行依赖，分发时复制整个目录。
- 应用窗口和 EXE 图标由 `20260817-100008.png` 生成，ICO 资源保存在 `src/DatasetManager.App/Assets/DatasetManager.ico`。
- 优先模块化和可复用，预计需求会持续变化。

## 初版技术决策

- WPF 目标框架：.NET 8，x64。
- 持久化目录固定为 EXE 同级的 `records`，使记录随程序目录一起备份和迁移；数据集、YOLO、权重及日志分别保存在 `catalog.json`、`yolo-operations.json`、`weights.json` 和 `logs`。
- 新版首次启动时从 `%LOCALAPPDATA%\DatasetManager` 复制缺失的旧记录，旧目录保留为迁移备份；后续只读写当前 EXE 旁的 `records`。
- `publish_standalone.bat` 使用临时发布目录，成功后继承原 `bin/Standalone/records` 再替换程序文件，发布失败不破坏现有独立版和记录。
- 组合清单保存在用户指定的 JSON 路径；复制创建的数据集根目录保存 `dataset.json` 和 `.dataset-manager-owned.json`。
- 数据集只登记绝对路径，不复制、移动或删除用户图片。
- 原始数据集、已处理数据集和已创建数据集支持按记录迁移到指定的新目录，可选择复制或剪切；迁移会同步更新主路径、已登记标注批次路径，以及已创建数据集 `dataset.json` 中的样本路径。主目录外的标签统一归档到新目录的 `annotations` 子目录。剪切采用先复制并保存记录、再删除原目录的顺序，清理失败时保留可用的新记录并提示残留路径。
- 派生关系当前为单父级：`ParentDatasetId`；模型未来可以改成关系表以支持多源合并。
- 类型当前为 `Raw`、`Test`、`Validation`。不要把 train/val/test 目录结构硬编码进扫描逻辑。
- LabelMe 标签路径不与图片路径绑定；每个标注批次递归扫描自己的标签目录并解析 shape。
- 图像扩展名由 `DatasetScanner` 集中维护。
- 目录扫描记录子文件夹总数，并按实际含图片目录统计各格式和图片总数。
- OpenCV 逐张读取并去重汇总图片分辨率、单通道位深和通道数；无法解码的图片单独计数。

## 下一阶段候选项

- LabelMe JSON 完整性检查（图片路径、尺寸、shape、类别统计）。
- 数据集详情中的图片/标签浏览器。
- 一个派生数据集关联多个来源。
- 标签类别字典、筛选和统计图。
- 数据库持久化、导入导出项目清单。
- 外部处理工具以命令行或插件方式登记处理记录，但不把转换逻辑耦合进核心层。

## 维护约定

每次需求或关键技术决策变化时更新本文件。实现细节以代码为准，本文件记录长期意图和边界。
